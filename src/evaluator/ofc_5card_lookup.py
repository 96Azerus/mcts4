# src/evaluator/ofc_5card_lookup.py v1.3
# -*- coding: utf-8 -*-
"""
...
class LookupTable:
    ...
    def _calculate_flushes(self):
        ...
        # Генерируем все возможные 5-карточные комбинации рангов
        ...
        # --- ИСПРАВЛЕНО: Присваиваем ранги стрит-флешам (1-10) в правильном порядке ---
        rank = 1
        for sf_bits in straight_flushes_rank_bits: # Используем предопределенный порядок
            prime_product = CardUtils.prime_product_from_rankbits(sf_bits)
            self.flush_lookup[prime_product] = rank
            rank += 1
        if rank -1 != LookupTable.MAX_STRAIGHT_FLUSH: # Проверяем, что последний ранг SF = 10
             logger.error(f"Assigned {rank-1} straight flush ranks, expected {LookupTable.MAX_STRAIGHT_FLUSH}.")

        # Присваиваем ранги обычным флешам
        ...
        # Вычисляем ранги для стритов и старших карт
    ...
    def _calculate_multiples(self):
        ...
        rank = LookupTable.MAX_STRAIGHT_FLUSH + 1 # 11
        for quad_rank_idx in backwards_ranks:
            kickers_indices = list(backwards_ranks); kickers_indices.remove(quad_rank_idx)
            for kicker_idx in sorted(kickers_indices, reverse=True): # Сортируем кикеры
        ...
        rank = LookupTable.MAX_FOUR_OF_A_KIND + 1 # 167
        for trip_rank_idx in backwards_ranks:
            pair_rank_indices = list(backwards_ranks); pair_rank_indices.remove(trip_rank_idx)
            for pair_rank_idx in sorted(pair_rank_indices, reverse=True): # Сортируем пару
        ...
        rank = LookupTable.MAX_STRAIGHT + 1 # 1610
        for trip_rank_idx in backwards_ranks:
        ...
        # 4) Две пары (Two Pair)
        rank = LookupTable.MAX_THREE_OF_A_KIND + 1 # 2468
        ...
        # 5) Пара (Pair)
        rank = LookupTable.MAX_TWO_PAIR + 1 # 3326
    ...
    def _get_lexographically_next_bit_sequence(self, bits: int) -> Generator[int, None, None]:
        ...
        while True:
            if next_val == 0: break
            try:
                rightmost_one = next_val & -next_val
                if rightmost_one == 0: break
                next_higher_one_bit = next_val + rightmost_one
...
if __name__ == '__main__':
    ...
    print(f"Rank for Wheel SF (prime {prime_wheel_sf}): {lookup.flush_lookup.get(prime_wheel_sf)}") # Должно быть 10
